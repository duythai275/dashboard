  ...
  return (
    <GeoJSON
      ...
      onEachFeature={(feature, layer) => {
        ...
        layer.on("mouseover", () => {
          setLabel(feature.properties.name + ": " + (foundData !== null ? foundData + postfix : t("noData")));
          layer.setStyle({
            weight: 3
          });
        });
...
const MarkersLayer = ({ individualMarkers }) => {
  return individualMarkers.map((marker, index) => {
    return <Marker key={index} position={marker.coordinates} icon={marker.icon}></Marker>;
  });
};

const MixedMap = (props) => {
  const { individualMarkers, markers } = props;
  ...
  return (
    <MapContainer
      ...
      // dragging={false}
    >
      {individualMarkers && individualMarkers.length > 0 && <MarkersLayer individualMarkers={individualMarkers} />}
      {markers && markers.length > 0 && <ClusterLayer markers={markers} />}
      <LegendLayer {...props} />
      <TileLayer
