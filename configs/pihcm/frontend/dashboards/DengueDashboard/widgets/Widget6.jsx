        ...
        data={{
          ...
          datasets: listLegend.map((legend, index) => ({
            ...
            data: data.ous.map((ou) => {
              ...
              const totalData = data.data.filter(
                (item) => item.dx === "laUqDdeLgDx" && item.ou === ou
              );
              const lestThan15Data = data.data.filter(
                (item) => item.dx === "hl9VTQA9Sor" && item.ou === ou
              );
              const equal15Data = data.data.filter(
                (item) => item.dx === "YY9B8VjuLqo" && item.ou === ou
              );
              if (legend === "<=15") {
