  ...
  useEffect(() => {
    (async () => {
      const resultCase = await pull(
        `/api/sqlViews/G0IHd6DtZDf/data?paging=false&var=table:_analytics_casereporting_cases_region&var=startYear:${LAST_YEAR}&var=endYear:${THIS_YEAR}`
      );

      const resultDeath = await pull(
        `/api/sqlViews/G0IHd6DtZDf/data?paging=false&var=table:_analytics_casereporting_deaths_region&var=startYear:${LAST_YEAR}&var=endYear:${THIS_YEAR}`
      );
