...
const apis = [
  ...
  },
  {
    ...
    handler: async (dhis2Apis, req) => {
      ...
      const result = await Promise.all([await dhis2Apis[0].get(dataUrl), await dhis2Apis[0].get(popLiveBirthUrl), await dhis2Apis[0].get(targetUrl)]);
  ...
  },
  {
    ...
    handler: async (dhis2Apis, req) => {
      ...
      const result = await Promise.all([await dhis2Apis[0].get(dataUrl), await dhis2Apis[0].get(targetUrl)]);
      const getValueIndex = (headers) => {
        return headers.findIndex((header) => header.name === "value");
      };
      const getPeIndex = (headers) => {
