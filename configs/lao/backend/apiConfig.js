...
const apis = [
  ...
  },
  {
    ...
    handler: async (dhis2Apis) => {
      const result = await dhis2Apis[0].get(
        "/api/analytics.json?dimension=dx:FSLrz90vXKf;cPcvesqWRtH;cwhEsbBe6Zs;kyVKK0JcRPJ;sISjKc2LEDg&dimension=pe:LAST_5_YEARS&filter=ou:IWp9dQGM0bS&includeNumDen=false&skipData=false&skipMeta=false"
      );
      const response = {};
      ...
      response.dxs = result.data.metaData.dimensions.dx;
      return response;
    }
  },
  {
