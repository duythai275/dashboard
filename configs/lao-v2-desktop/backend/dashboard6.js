const apisDashboard6 = [
  ...
  },
  {
    ...
    handler: async (dhis2Apis) => {
      const result = await dhis2Apis[0].get(
        `/api/analytics.json?dimension=dx:h4B7BYeWut1&filter=pe:LAST_12_MONTHS&dimension=ou:IWp9dQGM0bS;OU_GROUP-jblbYwuvO33&includeNumDen=false&skipData=false&skipMeta=false`
      );
      return result.data;
    },
  },
  {
    ...
    handler: async (dhis2Apis) => {
      const result = await dhis2Apis[0].get(
        `/api/analytics.json?dimension=dx:Gd5efmbROUX.REPORTING_RATE;Gd5efmbROUX.REPORTING_RATE_ON_TIME&filter=pe:LAST_12_MONTHS&dimension=ou:IWp9dQGM0bS;OU_GROUP-jblbYwuvO33&includeNumDen=false&skipData=false&skipMeta=false`
      );
      return result.data;
    },
  },
