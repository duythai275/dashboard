...
const Widget4_1 = ({ setLoading }) => {
  ...
  useEffect(() => {
    if (additionalState.widget1457Dashboard2Ready) {
      const response = {};
      response.data = additionalState.widget1457Dashboard2Data.rows.map(
        (row) => ({
          pe: row[1],
          item: row[0],
      ...
      response.pes =
        additionalState.widget1457Dashboard2Data.metaData.dimensions.pe
          .filter((item) => {
...
export default withWidgetChildrenLoader(Widget4_1);
