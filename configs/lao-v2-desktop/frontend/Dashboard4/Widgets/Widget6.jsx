...
const Widget6 = ({ setLoading }) => {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const resultData = await pull("/api/getDashboard4Widget6Data");
      const response = {};
  ...
  const formatLabel = (chart, index) => {
    ...
    const percent = ((chart.data.datasets[0].data[index] * 100) / sum).toFixed(2);
    const foundIndicator = indicators.find((indicator) => indicator.id === result.dx[index]);
    const indicatorsName = localeName === "En" ? foundIndicator.nameEn : foundIndicator.nameLo;
